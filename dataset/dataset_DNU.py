...
class DataSet_DNU(data.Dataset):
    """ -----  数据集文件夹简介  ---------
        文件使用官方数据集（文件名末尾带编号）
        每个 .mat 文件，每个文件中，有8行信号，分别代表:
        1-电机振动，2,3,4-行星齿轮箱在x, y, z三个方向的振动，
        5-电机转矩，6,7,8-平行齿轮箱在x, y, z三个方向的振动。
        第2,3,4行信号均有效。
    """
    ...
    def __init__(self, root, load_files=None, repeat_win=1.0, window=2048, label: int = None, num_class: int = None,
        ...
        super(DataSet_DNU, self).__init__()
    ...
    def load_file_onlyone(self, path: str, win: int, repeat_win: float, cut_same=False, cate: list = None,
        ...
        """ 官方数据中有存在放置错误的情况 """
        try:
            data = pd.read_csv(path,  skiprows=15)
            tmp = np.array(data.iloc[0, 1])
        except:
            data = pd.read_csv(path, sep='\t', skiprows=15)  ## 部分csv文件数据全在第一列，以\t相隔
            tmp = np.array(data.iloc[0, 1])
        ...
        # print('path:', path)
        # print('===', data.keys())
        # print('---',np.array(data.iloc[0,0]).split('\t'))
        # print('****', data.iloc[:,1].shape)
        if 1:
            tmp = np.array(data.iloc[:,1])

            if down_fre is not None:
                tmp = sample_frequency_change(tmp, ori_sample_frequency=20, tar_sample_frequency=12)
        ...
        if 1:
            tmp = np.array(data.iloc[:,2])
            if down_fre is not None:
                tmp = sample_frequency_change(tmp, ori_sample_frequency=20, tar_sample_frequency=down_fre)
            tmp_chunk = seq_chunk(tmp, chunk=win, step=int(win * (1 - repeat_win)),
                                  log=path +  '   ' + str(2),
                                  ishow=ishow)
...
class DataSet_DNU_Signal_Process(DataSet_DNU):
    """ -----  数据集文件夹简介  ---------
            文件使用官方数据集（文件名末尾带编号）
    ...
    def __init__(self, root, load_files=None, repeat_win=1.0, window=2048, label: int = None, num_class: int = None,
                 ...
                 snr: float = None):
        super(DataSet_DNU_Signal_Process, self).__init__(root, load_files, repeat_win, window, label, num_class,
                                                         train_test_rate, cate, down_fre)

        self.mask_win = mask_win
        self.mask_start_idx = mask_start_idx
        self.snr = snr
        self.args['program name'] = 'DataSet_DNU_Signal_Process'
...
class DataSet_DNU_MultiFile:
    ...
    def get_train_and_test_set_random(self, root, load_files: list = None, train_test_rate=0.7,
        ...
        for idx, file in enumerate(load_files):
            dataset = DataSet_DNU(root, [file], repeat_win, window, label=idx, num_class=num_class)
    ...
    def get_train_and_test_set_ordered_enhance(self, root, load_files: list = None, train_test_rate=0.7,
        ...
        arg0 = None
        for idx, file in enumerate(load_files):
            dataset = DataSet_DNU_Signal_Process(root, [file], repeat_win, window, label=idx, num_class=num_class, **kwargs)
            arg0 = dataset.config_make()
            train_num = int(len(dataset) * train_test_rate)
            test_num = len(dataset) - train_num
            # print('train_num, test_num :', train_num, test_num, train_num + test_num, len(dataset))

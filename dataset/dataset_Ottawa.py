...
class DataSet_Ottawa(data.Dataset):
    """ -----  数据集文件夹简介  ---------
        文件使用官方数据集（文件名为类别）   每个文件都是时变转速下的
    ...
    def __init__(self, root, load_files=None, repeat_win=1.0, window=2048, label: int = None, num_class: int = None,
        ...
        super(DataSet_Ottawa, self).__init__()
    ...
    def load_file_onlyone(self, path: str, win: int, repeat_win: float, cut_same=False, ishow=False):
        ...
        data0 = scio.loadmat(path)
        num_record = {'origal': {}, 'new': {}}
        tmplist = []
        data = data0['Channel_1']
        ...
        num_record['origal']['Channel_1'] = tmp.shape[0]
        num_record['new']['Channel_1'] = tmp_chunk.shape[0]

        data = data0['Channel_2']
        tmp = np.squeeze(data, axis=1)
        # print( tmp.shape)
        tmp_chunk = seq_chunk(np.squeeze(tmp), chunk=win, step=int(win * (1 - repeat_win)),
                              log=path + '  ',
                              ishow=ishow)
        tmplist.append(tmp_chunk)
        num_record['origal']['Channel_2'] = tmp.shape[0]
        num_record['new']['Channel_2'] = tmp_chunk.shape[0]
    ...
    def __getitem__(self, item):
        ...
        vibr, speed = dat[item, :, 0], dat[item, :, 1]
        label = label[item, 0]
        ...
        dat2 = torch.tensor(speed, dtype=torch.float).reshape(1, -1)
...
class DataSet_Ottawa_Signal_Process(DataSet_Ottawa):
    """ -----  数据集文件夹简介  ---------
        文件使用官方数据集（文件名末尾带编号）
    ...
    def __init__(self, root, load_files=None, repeat_win=1.0, window=2048, label: int = None, num_class: int = None,
                 ...
                 snr: float = None):
        super(DataSet_Ottawa_Signal_Process, self).__init__(root, load_files, repeat_win, window, label, num_class,
...
class DataSet_CRWU_MultiFile:
    ...
    def get_train_and_test_set_random(self, root, load_files: list = None, train_test_rate=0.7,
        ...
        for idx, file in enumerate(load_files):
            dataset = DataSet_Ottawa(root, [file], repeat_win, window, label=idx, num_class=num_class)
    ...
    def get_train_and_test_set_ordered_enhance(self, root, load_files: list = None, train_test_rate=0.7,
        ...
        arg0 = None
        for idx, file in enumerate(load_files):
            dataset = DataSet_Ottawa_Signal_Process(root, [file], repeat_win, window, label=idx, num_class=num_class,
                                                  **kwargs)
            arg0 = dataset.config_make()
            train_num = int(len(dataset) * train_test_rate)
            test_num = len(dataset) - train_num
...
def run():
    ...
    # dataset = DataSet_Ottawa('I:\python_datasets\CRWU\CRWU_original', None, 2, 500)

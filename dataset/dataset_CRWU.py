...
class DataSet_CRWU(data.Dataset):
    """ -----  数据集文件夹简介  ---------
        文件使用官方数据集（文件名末尾带编号）
    ...
    def __init__(self, root, load_files=None, repeat_win=1.0, window=2048, label: int = None, num_class: int = None,
        ...
        super(DataSet_CRWU, self).__init__()
        ...
        self.all_data_arr_DE_FE_BA_label = self.load_files_and_format(files, win=window, repeat_win=repeat_win,
    ...
    def load_file_onlyone(self, path: str, win: int, repeat_win: float, cut_same=False, ishow=False):
        ...
        # frequency, _, _, category, condition, num_name = filename[:-4].split('_')
        num_name = filename[:-4].split('_')[-1]
        # print(frequency, _, _, category, condition, num_name)
        """ 特殊情况 """
        if filename =='12k_Drive_End_B028_0_3005.mat': num_name='048'
        data = scio.loadmat(path)
        data = pd.DataFrame([data])
        num_record = {'origal': {}, 'new': {}}
        tmplist = []  # 0:DE  1:FE   2:BA
        # print('\t', path, data.keys())
        for idx in data.keys():
            if idx.endswith('_time') and num_name in idx:
                tmp = data[idx][0]
                if ishow: print('\t\tdat:', idx, '---', tmp.shape)
                tmp_chunk = seq_chunk(np.squeeze(tmp), chunk=win, step=int(win * (1 - repeat_win)),
                                      log=path + '  ' + idx,
                                      ishow=ishow)
                if 'DE' in idx:
                    for i in range(len(tmplist), 1): tmplist.append([])
                    tmplist[0] = tmp_chunk
                    num_record['origal']['DE'] = tmp.shape[0]
                    num_record['new']['DE'] = tmp_chunk.shape[0]
                elif 'FE' in idx:
                    for i in range(len(tmplist), 2): tmplist.append([])
                    tmplist[1] = tmp_chunk
                    num_record['origal']['FE'] = tmp.shape[0]
                    num_record['new']['FE'] = tmp_chunk.shape[0]
                elif 'BA' in idx:
                    for i in range(len(tmplist), 3): tmplist.append([])
                    tmplist[2] = tmp_chunk
                    num_record['origal']['BA'] = tmp.shape[0]
                    num_record['new']['BA'] = tmp_chunk.shape[0]
    ...
    def load_files_and_format(self, files: [], win: int, repeat_win: float, cut_same=False,
        ...
        for idx, file in enumerate(files):
            ...
            for i in range(len(datalist), 3):
                datalist.append(np.zeros_like(datalist[0]))
            # print( datalist[0].shape, datalist[1].shape,datalist[2].shape,)
            if label is None:
                label2 = idx
            else:
                label2 = label
            ...
            datalist = np.stack([datalist[0][:minnum], datalist[1][:minnum], datalist[2][:minnum]], axis=2)
            ...
            # print('data shape log:',label, all_data_arr_DE_FE_BA.shape, all_data_label.shape, all_data_arr_DE_FE_BA.shape[0] - datalist.shape[0])
    ...
    def __getitem__(self, item):
        ...
        DE, FE, BA = dat[item, :, 0], dat[item, :, 1], dat[item, :, 2]
        label = label[item][0]
        ...
        FE = torch.tensor(FE, dtype=torch.float).reshape(1, -1)
        BA = torch.tensor(BA, dtype=torch.float).reshape(1, -1)
        label = torch.tensor(label, dtype=torch.long)  # .reshape(1)
        # return label, DE, FE, BA  # self.idx[item]
        return label, torch.tensor([1], dtype=torch.float), torch.tensor([1], dtype=torch.float), DE, FE, BA
        pass
...
class DataSet_CRWU_Signal_Process(DataSet_CRWU):
    """ -----  数据集文件夹简介  ---------
        文件使用官方数据集（文件名末尾带编号）
        每个 .mat 文件就是一种故障类型，里边包含两种或三种信号数据
        示例：12k_Drive_End_B007_1_119.mat
    ...
    def __init__(self, root, load_files=None, repeat_win=1.0, window=2048, label: int = None, num_class: int = None,
                 train_test_rate:float=0.7, cate:str=None, mask_win:int=None, mask_start_idx:int=None, snr:float=None):
        super(DataSet_CRWU_Signal_Process, self).__init__(root, load_files, repeat_win, window, label, num_class,
                                                          train_test_rate, cate)

        # self.all_data_arr_DE_FE_BA_label
        # self.args = {
        #     'program name': 'DataSet_CRWU_Signal_Process',
        #     'program path': os.path.abspath(__file__),
        #     'root path': str(root),
        #     'load files': str(files),
        #     'repeat_win': repeat_win,
        ...
        #     'log': 'All file\'s data were uniformly normalized!',
        # }
        # self.all_data_arr_DE_FE_BA_label = [self.all_data_arr_DE_FE_BA_label[0], self.all_data_arr_DE_FE_BA_label[1]]
    ...
    def __getitem__(self, item):
        ...
        DE, real_rate, start_rate = self.signal_processing(DE, np.random.randint(0,3))
        # if real_rate<0.5:
        #     label = 10
        ...
        return label, rate, start_rate, DE, FE, BA # self.idx[item]
        pass
...
class DataSet_CRWU_MultiFile:
    ...
    def get_train_and_test_set_random(self, root, load_files: list = None, train_test_rate=0.7,
        ...
        for idx, file in enumerate(load_files):
            dataset = DataSet_CRWU(root, [file], repeat_win, window, label=idx, num_class=num_class)
    ...
    def get_train_and_test_set_ordered_enhance(self, root, load_files: list = None, train_test_rate=0.7,
        ...
        arg0 = None
        for idx, file in enumerate(load_files):
            dataset = DataSet_CRWU_Signal_Process(root, [file], repeat_win, window, label=idx, num_class=num_class, **kwargs)
            arg0 = dataset.config_make()
            train_num = int(len(dataset) * train_test_rate)
            test_num = len(dataset) - train_num
            # print('train_num, test_num :', train_num, test_num, train_num + test_num, len(dataset))
...
def run():
    ...
    # dataset = DataSet_CRWU('I:\python_datasets\CRWU\CRWU_original', None, 2, 500)

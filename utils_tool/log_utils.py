...
class Summary_Log:
    ...
    def __init__(self, path, headers: dict, save_csv=False, tm_str='', save_log=True, save_model=True,
    ...
    def add_scalars_(self, main_tag: str, scalars: {}, step: int, tolerant=False):
        ...
        ss = ''
        for k in scalars.keys(): ss += str(k)
        if self.separator in main_tag or self.separator in str(ss):
            raise Exception(f" main_tag: {main_tag} and scalars.keys: {scalars.keys()} can not contain the char: \"{self.separator}\", which was used in self.separator")
        ...
        ''' 定期写入 csv 文件 '''
        if self.save_csv and len(self.data_buff[self.csv_head[0]]) % 20 == 0:
            self.__save_csv_all(None, False)
        pass
    ...
    def __save_csv_all(self, file=None, ishow=True):
    ...
    def plot_output_in_fig(self, output, filename):
        output_np = output.cpu().detach().numpy()  # np.random.randn(32, 1, 2048)  #
        # print(output_np.shape)
        num_samples = len(output_np)
        # 定义每个子图的大小和间距
        plt.clf()
        fig, axs = plt.subplots(num_samples, 1, figsize=(8, 2*num_samples), sharex=True, sharey=True)

        plt.subplots_adjust(hspace=0., top=0.95, bottom=0.05, left=0.1, right=0.9)
        plt.xticks([])
        plt.yticks([])
        # 遍历每个样本，并在子图中绘制曲线
        for i in range(num_samples):
            axs[i].plot(output_np[i][0])
            # axs[i].set_title('Sample {}'.format(i+1))
            # axs[i].set_xlabel('Time')
            # axs[i].set_ylabel('Value')
        path_log = os.path.dirname( self.csv_log_file)
        filepath = os.path.join(path_log, 'output', str(filename)+'.jpg')
        ...
        try:
            ...
            for name in self.data_buff.keys():
                if len(self.data_buff[name]) > 0:
                    issave = True
            if issave: self.__save_csv_all(None, False)
        except:
            pass
        try:

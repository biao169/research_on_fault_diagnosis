...
class t_SNE_Visualization:
    ...
    def __check_data(self):
        ...
        else: outp = global_features

        if len(self.labels_list) != len(outp):
            print('\033[31m', f'data ERROR: hook\' size is {len(outp)}, label\'s size is {len(self.labels_list)}', '\033[0m',)
    ...
    def t_sne_plot(self, dat=None, label=None):
        ...
        for i in range(size):
            ...
            # print('==', idx)
            x = X[idx,:]
            plt.scatter(x[:,0], x[:,1], s=20,  c=colors[i], marker=maker[i],  #, cmap='brg' marker=maker[int(Y[i][0])],

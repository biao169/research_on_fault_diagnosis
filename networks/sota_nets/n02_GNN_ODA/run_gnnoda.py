...
from networks.sota_nets.n01_MSCNet.mscnet import MSCNet as Net
from networks.sota_nets.n01_MSCNet import train_mscnet as Train
...
def get_config(config=config, set='mfpt_set1'):
    ...
    config['optimizer'] = {}
    config['scheduler'] = {}
    config['optimizer']['Adam'] = dict(lr=0.003, betas=[0.9, 0.999])  # , weight_decay=0.0001
    # config['scheduler']['StepLR'] = dict(step_size=50, gamma=0.1, last_epoch=-1)
    config['train_loader']['dataset']['miniBatch'] = 400
...
def designed_train():
    ...
    train_dataset, test_dataset = DataSet_MFPT_MultiFile().get_train_and_test_set_ordered_enhance(
        ...
        mask_win=int(window * 0.2),
        mask_start_idx=None,
        snr=None)  # cate='test',  #  int(window * 0.6)  DataSet_CRWU_Signal_Process
...
def run():
    ...
    train_way = dict(
                    a=dict(network='lstm', dataset='mfpt_set1', train_size=window, evalute_szie=window),

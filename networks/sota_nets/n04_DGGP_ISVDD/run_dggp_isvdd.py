...
from networks.sota_nets.n04_DGGP_ISVDD.train_dggp_isvdd import Train
...
def get_config(config=config, set='mfpt_set1'):
    ...
    config['test_loader']['shuffle'] = False
    config['optimizer'] = {}
    # config['scheduler'] = {}
    config['optimizer']['Adam'] = dict(lr=0.01, betas=[0.9, 0.999], weight_decay=0.01)  #
    # config['scheduler']['StepLR'] = dict(step_size=50, gamma=0.1, last_epoch=-1)
    config['train_loader']['dataset']['miniBatch'] = 64
    config['test_loader']['dataset']['miniBatch'] = 64
...
def official_train():
    ...
    netGE = Generate_E(in_channel=1, out_len=2048)
    netGS = Generate_S(in_channel=1, out_len=2048)
    netDE = Discriminator_E(in_channel=1, out_len=2048)
    netDS = Discriminator_S(in_channel=1, out_len=2048)
    config, load_files = get_config()
    config['resultPath'] = os.path.join(config['resultPath'], r'{}_official'.format(netGE.log))
    ...
    trainloader = data.DataLoader(dataset=train_dataset, batch_size=config['train_loader']['dataset']['miniBatch'],
                                  shuffle=config['train_loader']['shuffle'], drop_last=True, )
    testloader = data.DataLoader(dataset=test_dataset, batch_size=config['test_loader']['dataset']['miniBatch'],
                                 shuffle=config['test_loader']['shuffle'], drop_last=True, )

    """ 删除日志文件 """
    delete_all_log_record_file(root=config['resultPath'], idx=[1], tm=None, rm_config=True, rm_csv=True,
                               rm_model=True, rm_tensor=True)
    config['epochNum'] = 8000
    train = Train(net=netGE, train_dataLoader=trainloader, test_dataLoader=testloader,
                  config=config, write_csv=True, save_log=True, tensorboard_mode='loss-acc',
                  new_thread=True)
    train.deploy_nets(generate_E=netGE, generate_S=netGS, discriminator_E=netDE, discriminator_S=netDS, batchSize=128)

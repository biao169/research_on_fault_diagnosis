...
from dataset.dataset_KAT import DataSet_KAT_MultiFile
from dataset.dataset_DNU import DataSet_DNU_MultiFile
from dataset.dataset_MFPT import DataSet_MFPT_MultiFile
from networks.my_nets.network_fn02 import mynet02 as Net
import networks.my_nets.network_fn02.train_mynet02 as Train
...
def dcnn_config(config=config, set='mfpt_set1'):
    ...
    config['optimizer']['Adam'] = dict(lr=0.01, betas=[0.7, 0.999], weight_decay=0.0001)
    config['scheduler']['ReduceLROnPlateau'] = dict(mode='min', factor=0.5, patience=10, eps=0.00000000001, cooldown=5)
...
def designed_train():
    # Fixme
    net = Net.Net2_Predict(in_channel=1, out_channel=num_class, net='mf')
    ...
    """ 删除日志文件 """
    delete_all_log_record_file(root=config['resultPath'], idx=[10], tm=None, rm_config=True, rm_csv=True,
...
def model_evaluation(mask_win=0,
                     mask_start_idx=0,
                     snr=10, model='', set='crwu_set1_2', is_auto=False):
    # Fixme
    net = Net.Net2_Predict(in_channel=1, out_channel=num_class, net='mf')
    ...
    if not is_auto:
        ...
        mask_start_idx = 0.4
        snr = 4
        model = 'checkpoint_2022-10-28 13.34.13_epo[553]_best.model'
    train_dataset, test_dataset = DataSet_MFPT_MultiFile().get_train_and_test_set_ordered_enhance(
...
def t_sne_vishow(mask_win=0, mask_start_idx=0, snr=0, model='', set='', is_auto=False, isave=False):
    ...
    # Fixme
    net = Net.Net2_Predict(in_channel=1, out_channel=num_class, net='gf')
    config, load_files = dcnn_config()
...
def run():
    ...
    """ 模型性能测试 """
    # model_evaluation(set='mfpt_set1')
    ...
    model = 'checkpoint_2022-12-06 15.43.15_epo[279]_best.model'
    eval_set = 'mfpt_set1'
    train_way = dict(
        a=dict(network='drfn-mf', dataset='mfpt_set1', train_size=2048, evalute_szie=2048),

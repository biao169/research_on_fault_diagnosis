...
class SelfAttention_Multihead(nn.Module):
    ...
    def __init__(self, num_attention_heads, input_size, hidden_size, attention_probs_dropout_prob, hidden_dropout_prob):
        super(SelfAttention_Multihead, self).__init__()
        if hidden_size % num_attention_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (hidden_size, num_attention_heads))
...
class Self_Attention(nn.Module):
    '''
    Scaled dot-product attention for DRFN
    '''

    def __init__(self, d_in, d_k, h, dropout=.1):
        '''
        ...
        d_in=1
        self.fc_o = nn.Sequential( nn.Linear(h * d_v, d_in), nn.ReLU6() )   #.ReLU() nn.Sigmoid()
        self.dropout = nn.Dropout(dropout)



    def forward(self, queries, attention_mask=None, attention_weights=None):
        '''
        Computes

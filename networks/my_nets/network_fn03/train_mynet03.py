...
class Train(Train_base):
    def __init__(self, net:nn.Module, train_dataLoader, test_dataLoader=None, num_classes=10, config:{}=None,  **kwargs):
        ...
        #nn.MSELoss().to(self.device)  ##nn.MSELoss() #
        pass
    ...
    def training_mode(self, pretrain=False, datasetName='crwu', **kwargs):
        ...
        kernel_size_train, kernel_value_train = True, False
        ...
        for idx_epo in range(start_epoch, self.epochNum):
            ...
            for i, dat in enumerate(self.train_dataLoader):
                ''' Design program '''
                if datasetName.lower() == 'crwu': label, rate, start_rate, de, fe, ba = dat[:6]
                elif datasetName.lower() == 'mfpt': label, _, _, de = dat[:4]
                de, label = de.to(device), label.to(device)
                output = net(de, kernel_size_train=kernel_size_train, kernel_value_train=kernel_value_train)
            ...
            if train_acc>0.7 and kernel_size_train and idx_epo<300 and not ( kernel_size_train and kernel_value_train):
                kernel_size_train, kernel_value_train = False, True
                print(f'[train] change trained value: kernel_size_train={kernel_size_train}, kernel_value_train={kernel_value_train}')
            elif train_acc>0.80 and idx_epo<200:
                kernel_size_train, kernel_value_train = True, True
                print(
                    f'[train] change trained value2: kernel_size_train={kernel_size_train}, kernel_value_train={kernel_value_train}')
            elif train_acc>0.959 and idx_epo<500: kernel_size_train, kernel_value_train = False, True
            elif train_acc > 0.959 and idx_epo < 700: kernel_size_train, kernel_value_train = True, True
    ...
    def test_model_visualization(self, net=None, dataLoader=None, pretrain=False, datasetName='crwu', **kwargs):
        ...
        for i, dat in enumerate(dataLoader):
            ...
            # print('--------', de.shape)
            # exit()
            # print('==',output.shape, label.shape, output.argmax(dim=1).shape )

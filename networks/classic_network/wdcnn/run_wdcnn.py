...
from networks.classic_network.wdcnn import wdcnn as Net
import networks.classic_network.wdcnn.train_wdcnn as Train
from utils_tool.log_utils import YamlHandler, delete_all_log_record_file

from report_make.excel_report import Make_Report

"""
    本部分程序只用于训练 复现 wdcnn ，经过验证，准确率和表现和论文一致。
    数据集读取后的划分模式有两种，这个对性能影响不大。
"""
...
datasetName = 'MFPT'
num_class = len(config['data_resource'][datasetName]['mfpt_set1'])
def wdcnn_config(config=config, set='mfpt_set1'):
    ...
    config['optimizer'] = {}
    config['scheduler'] = {}
    config['optimizer']['Adam'] = dict(lr=0.1, betas=[0.9,0.999], weight_decay=0.0001)
    config['scheduler']['ReduceLROnPlateau'] = dict(mode='min',factor=0.5,patience=30,eps=0.00000000001,cooldown=5)
    return config.copy(), load_files


def wdcnn_config2(config=config, set='mfpt_set1'):
    """ Only for evaluation in another condition or fault """
    load_files = config['data_resource'][datasetName][set]
    config, _ = wdcnn_config()

    return config, load_files

def official_train():
    # Fixme
    net = Net.WDCNN(in_channel=1, out_channel=num_class)
    config, load_files = wdcnn_config()
    ...
    """ 删除日志文件 """
    delete_all_log_record_file(root=config['resultPath'], idx=[3], tm=None, rm_config=True, rm_csv=True,
...
def designed_train():
    ...
    train_dataset, test_dataset = DataSet_MFPT_MultiFile().get_train_and_test_set_ordered_enhance(
                                        ...
                                        load_files=load_files,
                                        train_test_rate=0.5, repeat_win=0.85, window=2048,
                                        mask_win=int(2048 * 0.4),
                                        mask_start_idx=None,
                                        snr=None)  # cate='test',  #  int(2048 * 0.6)  DataSet_CRWU_Signal_Process
    config['train way'] = dict( mask_win=0.4, mask_start_idx=None, snr=None)
    print(f'train_dataset len = {len(train_dataset)},\t test_dataset len = {len(test_dataset)}')
...
def model_evaluation(mask_win=0,
                     mask_start_idx=0,
                     snr=10, model='', set='', is_auto=False):
    # Fixme
    net = Net.WDCNN(in_channel=1, out_channel=num_class)
    config, load_files = wdcnn_config(set=set)
    config['resultPath'] = os.path.join(config['resultPath'], r'{}_official'.format(net.log))
    """ 测试网络 """
    print(""" ============================================================ """)
    if not is_auto:
    ...
    testloader = data.DataLoader(dataset=test_dataset, batch_size=2048,
    ...
    result = train.test_model_visualization(dataLoader=testloader, pretrain=True,
                                            file=model, datasetName=datasetName
                                            )
...
def t_sne_vishow(mask_win=0, mask_start_idx=0, snr=0, model='', set='', is_auto=False, isave=False):
    ...
    # Fixme
    net = Net.WDCNN(in_channel=1, out_channel=10)
    config, load_files = wdcnn_config()
    ...
    if not is_auto:
        ...
        mask_start_idx = 0
        snr = 0
        model = 'checkpoint_2022-11-05 20.36.34_epo[337]_best.model'
    train_dataset, test_dataset = DataSet_MFPT_MultiFile().get_train_and_test_set_ordered_enhance(
        ...
        train_test_rate=0.5, repeat_win=0.85, window=2048,
        mask_win=int(2048 * mask_win),
        mask_start_idx=int(2048 * mask_start_idx) if mask_start_idx else None, #
        snr=snr)  # cate='test',  #  int(2048 * 0.6)  DataSet_CRWU_Signal_Process
    print('orignal size={}  new test_dataset size={}'.format(len(train_dataset), len(test_dataset)))

    testloader = data.DataLoader(dataset=test_dataset, batch_size=512*4,
                                 shuffle=False, drop_last=False, )
    print('new test_dataloader size={}'.format(len(testloader)))
...
def run():
    ...
    model = 'checkpoint_2022-11-30 20.51.01_epo[989]_best.model'
    eval_set = 'mfpt_set1'
    train_way = dict(
        a=dict(network='wdcnn', dataset='mfpt_set1', train_size=2048, evalute_szie=2048),
        b=dict(mask='0.2', mask_idx='random', SNR='random', eval_set=eval_set),   ##  random 0
    )
    dd = dict(save_file=os.path.join(config['resultPath'], 'wdcnn_record.xlsx' ),
              model_path=os.path.join(config['resultPath'], 'WDCNN_official', 'model', model ),
              train_way=train_way,
    ...
    for ms in mask_size:
        for mi in mask_idx:
            for sn in snr:
                print('running:', ms, '---mask idx:', mi, '---SNR:', sn)

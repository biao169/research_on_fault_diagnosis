...
from networks.classic_network.drsn import drsn as Net
...
def drsn_config(config=config, set='mfpt_set1'):
...
def official_train():
    # Fixme
    net = Net.DRSNet_CW(in_channel=1, out_channel=num_class)
    config, load_files = drsn_config()
    ...
    train_dataset, test_dataset = DataSet_MFPT_MultiFile().get_train_and_test_set_ordered_enhance(
                        ...
                        mask_start_idx=None,
                        snr=None)  # cate='test',  #  int(2048 * 0.6) random DataSet_CRWU_Signal_Process
...
def designed_train():
    ...
    config['train way'] = dict( mask_win=0.4, mask_start_idx=None, snr='random')
...
def model_evaluation(mask_win=0,
    ...
    # Fixme
    net = Net.DRSNet_CW(in_channel=1, out_channel=num_class)
    config, load_files = drsn_config(set=set)
...
def t_sne_vishow(mask_win=0, mask_start_idx=0, snr=0, model='', set='', is_auto=False, isave=False):
    ...
    """ 测试网络 """
    tsne = T_SNE()
    tsne.set_hook(net.classify_fc, 4)
    ...
    if not is_auto:
        ...
        mask_start_idx = 0
        snr = None
        model = 'checkpoint_2022-11-02 15.11.10_epo[112]_best.model'
    train_dataset, test_dataset = DataSet_MFPT_MultiFile().get_train_and_test_set_ordered_enhance(
...
def run():
    ...
    model = 'checkpoint_2022-11-30 21.08.33_epo[163]_best.model'
    eval_set = 'mfpt_set1'  # 'crwu_set1_2'
    train_way = dict(
        a=dict(network='drsn', dataset='mfpt_set1', train_size=2048, evalute_szie=2048),
        b=dict(mask='0.2', mask_idx='0', SNR='random', eval_set=eval_set), #   random  0
    )
    dd = dict(save_file=os.path.join(config['resultPath'], 'drsn_record.xlsx' ),
              model_path=os.path.join(config['resultPath'], 'DRSNet_CW_official', 'model', model ),
              train_way=train_way,

...
from networks.classic_network.lstm import lstm as Net
import networks.classic_network.lstm.train_lstm as Train
...
def lstm_config(config=config, set='mfpt_set1'):
    ...
    config['optimizer'] = {}
    config['scheduler'] = {}
    config['optimizer']['Adam'] = dict(lr=0.0001, betas=[0.9, 0.999], weight_decay=0.0001)
...
def model_evaluation(mask_win=0,
    ...
    net = Net.LSTM_1DC(in_channel=1, out_channel=num_class)
    config, load_files = lstm_config()
    if not (set is None or set == ''): config, load_files = lstm_config(set=set)
    config['resultPath'] = os.path.join(config['resultPath'], r'{}_official'.format(net.log))
...
def t_sne_vishow(mask_win=0, mask_start_idx=0, snr=0, model='', set='', is_auto=False, isave=False):
    ...
    train_dataset, test_dataset = DataSet_MFPT_MultiFile().get_train_and_test_set_ordered_enhance(
        ...
        train_test_rate=0.5, repeat_win=0.85, window=1024,
        mask_win=int(1024 * mask_win),
        mask_start_idx=int(1024 * mask_start_idx) if mask_start_idx else None, #
        snr=snr)  # cate='test',  #  int(2048 * 0.6)  DataSet_CRWU_Signal_Process
    print('orignal size={}  new test_dataset size={}'.format(len(train_dataset), len(test_dataset)))
...
def run():
    ...
    model = 'checkpoint_2022-11-30 20.59.43_epo[736]_best.model'
    eval_set = 'mfpt_set1'  # 'crwu_set1_2'
    train_way = dict(
                    a=dict(network='lstm', dataset='mfpt_set1', train_size=1024, evalute_szie=1024),
                    b=dict(mask='0.2', mask_idx='random', SNR='random', eval_set=eval_set),  #  random  0
                )
    dd = dict(save_file=os.path.join(config['resultPath'], 'lstm_record.xlsx' ),
              model_path=os.path.join(config['resultPath'], 'LSTM_1DC_official', 'model', model ),
              train_way=train_way,
